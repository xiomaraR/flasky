...
def read_all_cats():
    ...
    if breed_query:
        ...
        cats = Cat.query.filter_by(personality=breed_query)
    else:
        cats = Cat.query.all()
    cats_response = []
    cats = Cat.query.all()
...
def update_one_cat(id):
    ...
    db.session.commit
    return make_response(f"Cat # {cat.id} succesfully updated"), 200
...
def delete_one_cat(id):
    ...
    return make_response(f"Cat # {cat.id} succesfully deleted"), 200
